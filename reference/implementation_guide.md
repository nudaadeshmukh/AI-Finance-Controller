...
number. Persistent rules live in `CLAUDE.md`. **Deadline: 5 September 2026. Budget: ~4 days.**
...
sections it needs. This is the main lever on Claude Code usage across four days.
...
- `match/base.py` (Pass protocol), `match/constants.py`, `match/money.py`
  (**own copy** of `round_half_up` — see `CLAUDE.md` rule 2), `match/__init__.py`
  (`run_cascade`)
...
`results.json`. **Never widen one to lift a match rate** (`CLAUDE.md` rule 7).
...
## Implement
- `hypothesize/client.py` — Anthropic wrapper, `claude-haiku-4-5`, 20s timeout
