...
ONLY function in the codebase that writes `match_groups` (PROJECT_RULES.md rule 3).
...
def verify(proposal: MatchProposal, db: sqlite3.Connection, facts: DerivedFacts) -> ArithmeticProof:
    ...
    top level (to route cascade proposals through them, PROJECT_RULES.md rule 3),
...
def commit(proposal: MatchProposal, proof: ArithmeticProof, db: sqlite3.Connection) -> None:
    ...
    for member_key in proposal.member_keys:
        ...
        # (PROJECT_RULES.md rule 4) — clear any stale exception from an earlier run.
